...
func setupRoutes(g *fastglue.Fastglue, app *handlers.App, lo logf.Logger, basePath string, rdb *redis.Client, cfg *config.Config) {
	...
	g.POST("/api/clinic/waitlist", app.CreateClinicWaitlistEntry)
	g.PUT("/api/clinic/waitlist/{id}/remove", app.RemoveClinicWaitlistEntry)
	...
	g.GET("/api/clinic/appointments", app.ListClinicAppointments)
	g.PUT("/api/clinic/appointments/{id}/reschedule", app.RescheduleClinicAppointment)
	g.PUT("/api/clinic/appointments/{id}/cancel", app.CancelClinicAppointment)
