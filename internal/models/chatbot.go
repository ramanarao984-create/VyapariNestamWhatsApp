...
type ChatbotSettings struct {
	...
	WhatsAppAccount string    `gorm:"column:whatsapp_account;size:100;index" json:"whatsapp_account"` // References WhatsAppAccount.Name (empty for org-level defaults)
...
type AIContext struct {
	BaseModel
	OrganizationID  uuid.UUID   `gorm:"type:uuid;index;not null" json:"organization_id"`
	WhatsAppAccount string      `gorm:"column:whatsapp_account;size:100;index" json:"whatsapp_account"` // References WhatsAppAccount.Name (empty for org-level)
	Name            string      `gorm:"size:255;not null" json:"name"`
	IsEnabled       bool        `gorm:"default:true" json:"is_enabled"`
	Priority        int         `gorm:"default:10" json:"priority"`
