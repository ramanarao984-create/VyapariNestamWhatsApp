...
Production schema changes use embedded, versioned Goose SQL migrations. Re-running with `-migrate` is safe because Goose records applied versions in `goose_db_version`. The legacy GORM path is available only for local development with `-auto-migrate` and `app.environment = "development"`.
