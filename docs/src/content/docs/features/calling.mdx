...
Before enabling agents, open **Settings > Calling** and use **Calling
readiness**. It checks the local calling service, enabled WhatsApp accounts,
webhook token, credentialed TURN relay, public-network configuration, and UDP
range. A red item blocks local readiness; amber items are actions that must be
verified outside the app.

The diagnostic deliberately does **not** claim that Meta enrollment or webhook
delivery succeeded. Confirm Business Calling eligibility/enrollment for each
number and subscribe the production callback to all required calling events in
Meta before accepting real calls.
