...
<Aside type="caution">
  Canned Responses are **Quick Replies**: local reusable text for an open
  24-hour customer-service window. They are never submitted to Meta and cannot
  be used for campaigns, broadcasts, or scheduled messages. When the window
  is closed, select an approved WhatsApp template instead.
</Aside>
